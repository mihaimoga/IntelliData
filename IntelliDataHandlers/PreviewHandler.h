...
protected:
	...
	DECLARE_DOCUMENT(CIntelliDataDoc)
...
public:
	virtual HRESULT InitializeDocumentPreview(HWND hWndParent, RECT* prc)
	{
		...
		CCreateContext ctx;
		ctx.m_pNewViewClass = RUNTIME_CLASS(CIntelliDataView);
		...
		m_pDocument->AddRef();
		ctx.m_pCurrentDoc = DYNAMIC_DOWNCAST(CIntelliDataDoc, (CObject*) m_pDocument->GetContainer());
